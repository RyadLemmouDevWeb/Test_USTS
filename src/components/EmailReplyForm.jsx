...
const EmailReplyForm = ({ selectedEmail, onClose, onReplySent }) => {
  ...
  const handleManualReply = async () => {
    ...
    try {
      ...
      await emailService.sendManualReply(
        selectedEmail.id,
  ...
  const handleAutoReply = async () => {
    try {
      ...
      setError('Erreur lors de l\'envoi de la réponse automatique');
      console.error('Erreur envoi réponse auto:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateAIResponse = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_N8N_URL || 'http://localhost:5678'}/webhook/generate-reply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          originalContent: selectedEmail.summary || selectedEmail.content,
          context: replyText,
